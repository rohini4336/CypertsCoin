...
//unsigned int nTargetSpacing = 1 * 60; // 120 seconds
unsigned int nTargetSpacing = 2 * 60; // 120 seconds(2 min)
unsigned int nRetarget = 30;
//unsigned int nStakeMinAge = 15 * 24 * 60 * 60; // 10 hours
unsigned int nStakeMinAge = 2* 60 * 60;//2 hours
//unsigned int nStakeMaxAge = -1; // 100 hours
unsigned int nStakeMaxAge = 30 * 24 * 60 * 60; // 100 hours
...
const string strMessageMagic = "CypertsCoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // CypertsCoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // CypertsCoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    int64_t nRewardCoinYear = POS_STAKE_REWARD; // 10% per month


  /*  if(nBestHeight < 259200)
        nRewardCoinYear = 12 * nRewardCoinYear;
    else if(nBestHeight < 518400)
        nRewardCoinYear = 9.6 * nRewardCoinYear;
    else if(nBestHeight < 777600)
        nRewardCoinYear = 7.2 * nRewardCoinYear;
    else if(nBestHeight < 788400)
        nRewardCoinYear = 4.8 * nRewardCoinYear;
    else if(nBestHeight < 1036800)
        nRewardCoinYear = 3.6 * nRewardCoinYear;
    else if(nBestHeight < 1051200)
        nRewardCoinYear = 1.2 * nRewardCoinYear;
    else if(nBestHeight < 1555200)
        nRewardCoinYear = 0.6 * nRewardCoinYear;
    else if(nBestHeight < 1814400)
        nRewardCoinYear = 0.48 * nRewardCoinYear;
    else
        nRewardCoinYear = 0.42 * nRewardCoinYear;*/
...
// CypertsCoin: find last block index up to pindex
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // CypertsCoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // CypertsCoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        ...
        // CypertsCoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // CypertsCoin: track money supply and mint amount info
...
// CypertsCoin: total coin age spent in transaction, in the unit of coin-days.
...
// CypertsCoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // CypertsCoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // CypertsCoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // CypertsCoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // CypertsCoin: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // CypertsCoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // CypertsCoin: check proof-of-stake
    ...
    // CypertsCoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // CypertsCoin: getblocks may not obtain the ancestor block rejected
    ...
    // CypertsCoin: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "CypertsCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "CypertsCoin Era starts from 14 oct 2019";
        CTransaction txNew;
        txNew.nTime = 1571051759;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("e10d5bb2934a8ce0e052432a25f0b6a8324004e875ee3997fb71ac8de6be28e7"));
        block.print();
        ...
        // CypertsCoin: initialize synchronized checkpoint
...
unsigned char pchMessageStart[4] = {0xb9, 0x42, 0x75, 0xd1};
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // CypertsCoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // CypertsCoin: tell downloading node about the latest block if it's
